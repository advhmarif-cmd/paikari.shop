...
class VendorDashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final unreadBuyerChats = ref
            .watch(vendorChatConversationsProvider)
            .valueOrNull
            ?.where((chat) => chat.isUnreadFor(vendorUserId))
            .length ??
        0;
    ...
    return Scaffold(
            ...
            children: [
              IconButton(
                ...
                onPressed: () => Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (_) => const ChatInboxScreen(isVendor: true))),
              ),
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          await ref.read(myLocalProductsProvider.future);
        },
        child: ListView(
          ...
          children: [
            vendorAsync.when(
              ...
              error: (_, __) => const _VendorSummary(
                  storeName: 'Vendor store', status: 'pending'),
            ),
            const SizedBox(height: 20),
            const Text('আপনার products',
            ...
            productsAsync.when(
              ...
              data: (products) => products.isEmpty
                  ...
                  : Column(
                      children: products
                          .map(
                              (product) => _VendorProductTile(product: product))
                          .toList()),
            ),
            const SizedBox(height: 24),
            const Text('Buyer inquiries',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            ref.watch(vendorInquiriesProvider).when(
                  loading: () => const LinearProgressIndicator(),
                  error: (error, stack) =>
                      Text('Inquiry load করা যায়নি: $error'),
                  data: (inquiries) => inquiries.isEmpty
                      ? const Text('এখনও কোনো bulk inquiry নেই',
                      ...
                      : Column(
                          children: inquiries
                              .map((inquiry) => _InquiryTile(
                                  inquiry: inquiry,
                                  onRespond: () =>
                                      _respondToInquiry(context, ref, inquiry)))
                              .toList()),
                ),
            const SizedBox(height: 24),
            const Text('Vendor orders',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            ref.watch(vendorOrdersProvider).when(
                  ...
                  data: (orders) => orders.isEmpty
                      ? const Text('এখনও কোনো vendor order নেই',
                          style: TextStyle(color: Colors.grey))
                      : Column(
                          children: orders
                              .map((order) => _VendorOrderTile(
                                  order: order,
                                  onAdvance: () =>
                                      _advanceVendorOrder(context, ref, order),
                                  onCancel: () =>
                                      _cancelVendorOrder(context, ref, order)))
                              .toList()),
                ),
            const SizedBox(height: 24),
            ...
            ref.watch(vendorReturnRequestsProvider).when(
                  loading: () => const LinearProgressIndicator(),
                  error: (error, stack) =>
                      Text('Return load করা যায়নি: $error'),
                  data: (returns) => returns.isEmpty
                      ? const Text('এখনও কোনো return request নেই',
                          style: TextStyle(color: Colors.grey))
                      : Column(
                          children: returns
                              .map((request) => _VendorReturnTile(
                                  request: request,
                                  onRespond: () =>
                                      _respondToReturn(context, ref, request)))
                              .toList()),
                ),
            const SizedBox(height: 24),
            const Text('Quotation requests',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            ref.watch(vendorQuotationsProvider).when(
                  ...
                  data: (quotes) => quotes.isEmpty
                      ? const Text('এখনও কোনো quotation request নেই',
                          style: TextStyle(color: Colors.grey))
                      : Column(
                          children: quotes
                              .map((quote) => _VendorQuoteTile(
                                  quote: quote,
                                  onRespond: () =>
                                      _respondToQuote(context, ref, quote)))
                              .toList()),
                ),
          ],
        ),
...
Future<void> _respondToInquiry(
    BuildContext context, WidgetRef ref, ProductInquiry inquiry) async {
  final responseController =
      TextEditingController(text: inquiry.vendorResponse ?? '');
  String selectedStatus =
      inquiry.status == 'open' ? 'responded' : inquiry.status;
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              await ref.read(inquiryRepositoryProvider).respondToInquiry(
...
Future<void> _advanceVendorOrder(
    BuildContext context, WidgetRef ref, VendorOrder order) async {
  const nextStatus = {
    'pending': 'confirmed',
  ...
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Order status $next হয়েছে'),
        behavior: SnackBarBehavior.floating));
  } catch (error) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Status update করা যায়নি: $error'),
        behavior: SnackBarBehavior.floating));
  }
}

Future<Map<String, String?>?> _collectShipmentDetails(
        ...
        FilledButton(
          onPressed: () {
            ...
            Navigator.pop(dialogContext, {
              'courier':
                  courier.text.trim().isEmpty ? null : courier.text.trim(),
              'trackingNumber': trackingNumber.text.trim().isEmpty
                  ? null
                  : trackingNumber.text.trim(),
              'trackingUrl': trackingUrl.text.trim().isEmpty
                  ? null
                  : trackingUrl.text.trim(),
            });
          },
...
Future<void> _cancelVendorOrder(
  ...
  final confirmed = await showDialog<bool>(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      content:
          const Text('Shipment-এর আগে cancel করলে reserved stock release হবে।'),
      actions: [
        TextButton(
  ...
  try {
    await ref
        ...
        .updateVendorOrderStatus(vendorOrderId: order.id, status: 'cancelled');
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Vendor order cancel হয়েছে এবং stock release হয়েছে'),
        behavior: SnackBarBehavior.floating));
  } catch (error) {
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Vendor order cancel করা যায়নি: $error'),
        behavior: SnackBarBehavior.floating));
  }
}

Future<void> _respondToReturn(
    BuildContext context, WidgetRef ref, ReturnRequest request) async {
      ...
      builder: (context, setState) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              try {
                await ref.read(returnRepositoryProvider).respond(
                ...
                if (dialogContext.mounted) {
                  ScaffoldMessenger.of(dialogContext).showSnackBar(SnackBar(
                      content: Text('Return response save করা যায়নি: $error')));
                }
              }
            },
            child: const Text('Save'),
...
Future<void> _respondToQuote(
    BuildContext context, WidgetRef ref, QuotationRequest quote) async {
  final price = TextEditingController();
  final quantity =
      TextEditingController(text: quote.requestedQuantity.toString());
            ...
            children: [
              ...
              TextFormField(
                ...
                validator: (value) {
                  ...
                  return parsed == null || parsed < 0 ? 'সঠিক দাম দিন' : null;
                },
              ),
              TextFormField(
                controller: delivery,
              ...
              TextFormField(
                  ...
                  maxLines: 3,
                  decoration:
                      const InputDecoration(labelText: 'Vendor message')),
            ],
          ),
        ),
      ),
          ...
          onPressed: () async {
            ...
            try {
              await ref.read(quotationRepositoryProvider).respond(
                    ...
                    quotedQuantity: int.parse(quantity.text.trim()),
                    quotedUnitPrice: double.parse(price.text.trim()),
                    deliveryCharge: double.parse(delivery.text.trim()),
              ...
              if (dialogContext.mounted) {
                ScaffoldMessenger.of(dialogContext).showSnackBar(
                  SnackBar(content: Text('Quote পাঠানো যায়নি: $error')),
                );
              }
            }
          },
...
class _VendorReturnTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final canRespond =
        {'requested', 'approved', 'received'}.contains(request.status);
    return Card(
      ...
      child: ListTile(
        ...
        leading:
            const CircleAvatar(child: Icon(Icons.assignment_return_outlined)),
        title: Text('${request.status} · ${request.quantity} unit',
            style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle:
            Text(request.reason, maxLines: 2, overflow: TextOverflow.ellipsis),
        trailing: canRespond
            ? TextButton(onPressed: onRespond, child: const Text('Respond'))
            : null,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        trailing: quote.status == 'open' || quote.status == 'declined'
            ? TextButton(onPressed: onRespond, child: const Text('Quote'))
            : null,
      ),
    );
  }
...
class _VendorOrderTile extends StatelessWidget {
  ...
  const _VendorOrderTile(
      {required this.order, required this.onAdvance, required this.onCancel});

  @override
  Widget build(BuildContext context) {
    final canAdvance = {'pending', 'confirmed', 'processing', 'shipped'}
        .contains(order.status);
    final canCancel =
        {'pending', 'confirmed', 'processing'}.contains(order.status);
    return Card(
      margin: const EdgeInsets.only(bottom: 10),
      elevation: 0,
      shape: RoundedRectangleBorder(
      ...
      child: ListTile(
        ...
        title: Text(order.vendorStoreName,
            style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle: Text(
            '${order.items.length} line · ৳${order.totalAmount.toStringAsFixed(0)} · ${order.status}${order.trackingNumber == null ? '' : '\\n${order.courierName ?? order.courierProvider ?? 'Courier'}: ${order.trackingNumber}${order.courierStatus == null ? '' : ' · ${order.courierStatus}'}'}'),
        trailing: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            if (canAdvance)
              TextButton(
                  onPressed: onAdvance, child: const Text('Next status')),
            if (canCancel)
              TextButton(
                  onPressed: onCancel,
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
        leading: const CircleAvatar(child: Icon(Icons.request_quote_outlined)),
        title: Text('${inquiry.requestedQuantity} units · ${inquiry.status}',
            style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle:
...
class _VendorSummary extends StatelessWidget {
  ...
  const _VendorSummary(
      {required this.storeName, required this.status, this.city});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const CircleAvatar(
              radius: 26, child: Icon(Icons.storefront_outlined)),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                    '${city ?? 'বাংলাদেশ'} · ${isVerified ? 'Verified supplier' : 'Approval pending'}',
                    style:
...
class _VendorProductTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final approvalLabel =
        product.approvalStatus == 'approved' ? 'Approved' : 'Pending review';
    return Card(
      margin: const EdgeInsets.only(bottom: 10),
      elevation: 0,
      shape: RoundedRectangleBorder(
      ...
      child: ListTile(
        ...
        subtitle: Padding(
          ...
          child: Text(
              '৳${product.retailPrice.toStringAsFixed(0)} · MOQ ${product.moq} · $approvalLabel'),
        ),
        trailing: Icon(
            product.isAvailable
    ...
    return Container(
      ...
      child: const Column(
        children: [
          ...
          SizedBox(height: 8),
          Text('এখনও কোনো local product নেই',
              style: TextStyle(fontWeight: FontWeight.w800)),
          SizedBox(height: 4),
          Text('আপনার প্রথম product যোগ করুন।',
              style: TextStyle(color: Colors.grey)),
        ],
      ),
    );
...
class _VendorProductFormScreenState
        ...
        child: ListView(
          ...
          children: [
            ...
            SwitchListTile.adaptive(
              ...
              title: const Text('দরদাম করা যাবে',
                  style: TextStyle(fontWeight: FontWeight.w800)),
              value: _negotiable,
            ...
            Container(
              ...
              child: const Text(
                  'নতুন product approval-এর জন্য pending থাকবে এবং admin approval ছাড়া public catalog-এ দেখা যাবে না।',
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                label: Text(
                    _saving ? 'জমা হচ্ছে...' : 'Approval-এর জন্য জমা দিন',
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Product approval-এর জন্য জমা হয়েছে'),
          behavior: SnackBarBehavior.floating));
      Navigator.pop(context);
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Product save করা যায়নি: $error'),
          behavior: SnackBarBehavior.floating));
    } finally {
