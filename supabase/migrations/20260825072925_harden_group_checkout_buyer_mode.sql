-- Prevent a mixed B2B/B2C cart from being silently priced and stored as one mode.
-- The current order-group contract has one buyer_mode, so every submitted line
-- must agree with that server-authoritative group mode.
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) as items(value) loop
    v_item_buyer_mode := coalesce(nullif(v_item->>'buyerMode', ''), v_buyer_mode);
    if v_item_buyer_mode not in ('b2c', 'b2b') or v_item_buyer_mode <> v_buyer_mode then
      raise exception 'Mixed buyer modes are not supported in one cart';
    end if;
