...
The `sync-origen-catalog` Supabase Edge Function is designed for JWT verification with an admin-only JWT fallback and an optional server-to-server `x-origen-sync-secret` header backed by the `ORIGEN_SYNC_SECRET` Edge Function secret. It fetches active products from `https://origen-prime.vercel.app/api/products`, upserts shared rows, and deactivates previously synced Origen products that are no longer returned. The current function defaults to that URL but accepts an `ORIGEN_CATALOG_URL` secret for future configuration.
...
Before live launch, configure Supabase Auth providers for Phone OTP and Google OAuth, optionally enable Facebook OAuth, and register the exact native redirect URI `io.paikari.shop://login-callback/`. Replace the two zero-valued rows in `shipping_settings` with the actual inside-city and outside-city delivery charges. Keep Edge Function JWT verification enabled, restrict catalog sync to the admin JWT or a strong `ORIGEN_SYNC_SECRET`, and set `ORIGEN_CATALOG_URL` only if the default Origen endpoint should be overridden.
